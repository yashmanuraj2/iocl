...
const onEditAdmin = ({id, adminReason}) => {
  setInEditMode({
      status: true,
      rowKey: id
  })
  setRejectAdmin(adminReason);
...
{ 
 ...
 setmyrec((newDataArr) => {
  const indexOfItemToRemove = newDataArr.findIndex(
  ...
  return [
    ...newDataArr.slice(0, indexOfItemToRemove),
    ...newDataArr.slice(indexOfItemToRemove + 1),
  ];
});
*/
let myindexofitem = allData[0].findIndex(x => x.id === item.id);
 ...
 `${user.Designation==="HOD" ? await axios.put("http://localhost:5000/history/admin/"+ 4200 , allData[0][myindexofitem]) : null}`]);
 

}

/*
const onApproveAdmin =async(item) =>
{
...
allData[0][myindexofitem].rejectHOD = rejectHOD;
if(user.Designation==="ADMIN")
allData[0][myindexofitem].admin_status = 0;
allData[0][myindexofitem].rejectAdmin = rejectAdmin;
if(props.show!==false)
 return(

  <div>
    <input type="text" placeholder="Enter Reason" onChange={handleRejectHOD} />
...
<table>
      ...
      <thead>{historyData[0].map((item) => (
        <tr>       
          ...
          {`${item.rejectHOD!=="" ? <th>HOD Rejection Reason </th>: ""}`}
          {`${item.rejectAdmin!=="" ? <th>Admin Reject Reason</th> :""}`}
 ...
 console.log(historyData.length)
  ...
  return (
    <>
    <Header
          text={` Welcome Mr.${user.name}, ${user.Designation}`}
